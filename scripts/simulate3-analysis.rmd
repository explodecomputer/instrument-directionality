...
temp <- temp %>% as.data.frame() %>% select(simid, Method)
...
badid <- apply(temp2, 1, function(x) any(is.na(x)))
...
training_index <- sample(1:nrow(method_choices), nrow(method_choices) * 0.66, replace=FALSE)
testing_index <- c(1:nrow(method_choices))[! c(1:nrow(method_choices)) %in% training_index]

training <- method_choices[training_index, ]
testing <- method_choices[testing_index, ]
...
method.model.rf <- randomForest(Method ~ nsnp + nout + nexp, data=training, ntree=5000, mtry=15, importance=TRUE)
method.model.ctree <- ctree(Method ~ ., data=training)
method.model.lda <- lda(Method ~ ., data=training)
