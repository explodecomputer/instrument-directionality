...
ggplot(rocs, aes(x=method, y=auc, group=strategy, colour=strategy)) +
...
ggplot(rocs2, aes(x=method, y=auc, group=strategy, colour=strategy, shape=top)) +
geom_point() +
...
o <- out$coef %>% as_data_frame %>% 
	mutate(var = rownames(out$coef), f=`t value`^2, rsq=f/(f+out$df[2]-df[3])) %>% slice(-1) %>%
	arrange(desc(f))
o$var <- as.factor(o$var)
o$var <- factor(o$var, levels=as.character(o$var))
ggplot(o, aes(x=var, y=cumsum(rsq*out$r.squared/sum(rsq)))) +
